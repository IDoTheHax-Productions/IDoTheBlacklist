...
class ConfirmButton(ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if self.blacklist_data.get('minecraft_username'):
            mc_info['username'] = self.blacklist_data['minecraft_username']
            if not self.blacklist_data.get('minecraft_uuid'):
                self.blacklist_data['minecraft_uuid'] = await fetch_minecraft_uuid(mc_info['username'])
            mc_info['uuid'] = self.blacklist_data.get('minecraft_uuid', '')
...
Reason: Griefing and using hacks"""
    ...
    async def on_thread_create(self, thread):
        if isinstance(thread.parent, discord.ForumChannel):
            ...
            try:
                ...
                blacklist_channel_ids = [1345490362981945376, 1343190729597653023]  # Replace with your actual channel ID
    ...
    async def parse_blacklist_request(self, content):
        ...
        # Fetch UUID if Minecraft username is provided but UUID is missing
